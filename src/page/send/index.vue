  ...
  <div class="w-full h-full bg-white shadow-2xl  overflow-y-scroll p-2" id="ershou_body" @scroll="scrollBottm">
    ...
    <div class="mt-12 flex  ">
     ...
     h-44 rounded-xl  bg-gradient-to-r from-purple-500 accent-pink-200 to-blue-300 ">
        ...
        <div class="text-white text-xs pingfang mt-2">There are so many people around me who use the second-hand trading
          platform</div>
        <div class="flex h-12 mt-4 justify-end">
          <img :src="item.src" class="w-8 cursor-pointer transition-all hover:mt-3 h-8 rounded-full" :style="{
            transform: `translate(-${index * 15}px,0px)`
          }" alt="" v-for="(item, index) in head_arr">
        </div>
      </div>
    ...
    <div class="text-xl pingfang_jian mb-3 text-purple-500 font-bold mt-12 ml-5 border-b-2 w-10 pb-2 border-purple-400">
      广场</div>
