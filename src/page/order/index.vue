 ...
 * @LastEditTime: 2023-01-20 09:53:21
 * @Description: 
 * 
...
<div class="w-full p-2 h-full bg-cover bg-no-repeat  overflow-hidden overflow-y-scroll "
    ...
    <div class="flex items-center w-full  m-2 pb-3 border-b" v-for="(item,index) in order" :key="index">
      ...
      <div class="pingfang_jian text-gray-500 text-xs w-44  overflow-ellipsis overflow-hidden whitespace-nowrap  hover:whitespace-pre-wrap ">地址:{{ item.custorm_address }}
      </div>

     
        <n-button class="ml-3" @click="editOrder(item._id,1)"  v-if="item.status==0">发货</n-button>
      <n-button class="ml-3" @click="editOrder(item._id,2)" :disabled="id!=item.user_id" v-if="item.status==1">收货</n-button>
