  ...
  <div class="w-full p-2 bg-white overflow-hidden overflow-y-scroll">
    ...
    <div class="mt-7 flex w-full  border rounded-xl p-2" v-for="(item, index) in shop" :key="index">
      ...
      <div class="ml-3 flex w-full p-2 flex-col pingfang_jian justify-between ">
        ...
        <div class="text-right text-xl font-bold flex  items-center justify-end mr-2cb  dcadrfv">{{ item.price }}*<span
            class="pingfang_jian text-xs p-2 text-gray-500">{{ item.count }}</span></div>
      </div>
    </div>
    ...
    <div class="flex items-end pingfang_jian justify-between mt-12 mr-3 ">
      <div>总价:{{ sum }}</div>
      <div @click="jump('/success', '支付')"
        class="p-2 active:scale-50 transition cursor-pointer w-24 text-center rounded-xl bg-purple-500 text-white ">提交订单
      </div>
